...
function NewVenueContent() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (venue) => {
      toast.success('Заведение создано', { description: 'Отправьте на модерацию чтобы опубликовать' })
  ...
  // Геокодирование адреса → координаты + точка на карте (правка #45)
        ...
        <CardContent className="space-y-4">
            ...
            <div className="sm:col-span-2">
              ...
              <div className="mt-1 flex gap-2">
                <Input id="address" value={form.address} onChange={set('address')}
                  ...
                  className={`flex-1 ${errors.address ? 'border-destructive focus-visible:ring-destructive' : ''}`} />
                <Button type="button" variant="outline" className="shrink-0" onClick={handleGeocode} disabled={geocoding || !form.address.trim()}>
          ...
          <div className="pt-2">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              После создания заведение будет в статусе «Черновик». Отправьте на модерацию для публикации.
            </p>
          </div>
        </CardContent>
      </Card>
...
export default function NewVenuePage() {
  return <Suspense><NewVenueContent /></Suspense>
}
